...
class CleanPlan:
    ...
    def __init__(self, reg: Registry, modules: list[str]):
        self.reg = reg
        self.modules = set(modules)
        self.parents: list[DocType] = [
            d
            for d in reg.doctypes
            if d.module in self.modules and not d.istable and not d.issingle
        ]
    ...
    def table_for_link(self, target: str) -> str | None:
        ...
        if t is None or t.issingle:
            return None
        if not t.istable:
            return t.sql_table if target in self.parent_names else None
...
def clean_table(plan: CleanPlan, table: str, d: DocType, parent: DocType | None, parentfield: str):
    ...
    for f in d.columns:
        ...
        if f.is_link:
            ...
            why = (
                "frappe core doctype"
                if not reg.is_internal(f.options)
                else f"module {reg.get(f.options).module} out of scope"
                if not reg.get(f.options).istable
                else "child table with multiple parents"
            )
            null = " NOT NULL" if f.reqd else ""
            cols.append(
...
def emit_clean_bundle(reg: Registry, modules: list[str]) -> tuple[str, str, dict]:
    ...
    tables_sql = [
        ...
        "-- Load this file first, then foreign_keys.sql.",
        "-- ============================================================",
        "",
    ]
    all_fks: list[str] = []
